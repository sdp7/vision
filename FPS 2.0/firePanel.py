...
#Turns the system on
def system_on():
    for x in [reset_button, sys_on]:
        x.config(state="disabled")

    for x in [raise_alert, fault_button]:
        x.config(state="normal")
...
def alert():
    ...
    system_update.raise_alarm(str(floor_input.get()))
    window.after(500, record)
...
def reset():
    ...
    LEDS["fire_led"].turn_off()
    location_label.config(text= "Reset Fire Panel")
    LEDS["reset_led"].turn_on()

    window.after(2000, reset_2)
...
if __name__ == "__main__":
    ...
    left_label_y = 140
    right_label_y = 140
    for x in ["Power Fault", "System Fault", "Delay", "Test", "Reset"]:
    ...
    leds_x, leds_y = 65, 75
    for x in ["fault_led", "power_fault_led", "system_fault_led", "delay_led", "test_led", "reset_led"]:
